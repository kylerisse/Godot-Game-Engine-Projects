...
func _ready():
	...
	$Items.hide()
	set_camera_limits()
	var reddoor_id = $Walls.tile_set.find_tile_by_name('door_red')
	for cell in $Walls.get_used_cells_by_id(reddoor_id):
		reddoors.append(cell)
	spawn_items()
	$Player.connect('dead', self, 'game_over')
	$Player.connect('grabbed_key', self, '_on_Player_grabbed_red_key')
	$Player.connect('grabbed_key', self, '_on_Player_grabbed_green_key')
